...
def __convert_audio(in_path, out_path):
    ...
    """
    cmd = ["/home/khlu/sph2pipe_v2.5/sphpipe", "-f", "wav", in_path, out_path]
    subprocess.run(cmd)
...
def main():
    ...
    __process_set(
        os.path.join(data_root, "LDC2004S13", "fisher_eng_tr_sp_d*", "audio", "*", "*.sph",),
        os.path.join(dest_root, "LDC2004S13", "audio_wav"),
    )

    logging.info("\n\nConverting audio for Part 2")
    __process_set(
        os.path.join(data_root, "LDC2005S13", "fe_03_p2_sph*", "audio", "*", "*.sph",),
        os.path.join(dest_root, "LDC2005S13", "audio_wav"),
    )
